...
// Varios modelos gratis con visión, por si alguno se queda sin cuota o
// deja de estar disponible: probamos el siguiente automáticamente.
const SCAN_MODELS = [
...
async function callOpenRouterWithFallback(messages, extra = {}){
  let lastErr;
  for (const model of SCAN_MODELS) {
    try {
      return await callOpenRouter(model, messages, extra);
    } catch (err) {
      lastErr = err; // probamos el siguiente modelo de la lista
    }
  }
  throw lastErr;
}
