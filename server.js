/* ============================================================
   ...
   ------------------------------------------------------------
   Ahora usando Groq en vez de Gemini.
     POST /api/scan    -> analiza foto de comida (Groq, qwen/qwen3.6-27b, visión)
     POST /api/coach   -> chat fitness/nutrición (Groq, openai/gpt-oss-120b)
     POST /api/redeem  -> valida códigos premium

   Variable de entorno necesaria: GROQ_API_KEY
...
app.get("/", (req, res) => {
  res.json({ ok: true, service: "myfitai-backend (Groq)" });
});
...
markdown ni backticks:
{
  ...
  "items": [
    ...
    const data = await callGroq("qwen/qwen3.6-27b", [
      {
        role: "user",
        content: [
    ...
    if (!text) throw new Error("Groq no devolvió resultado.");
...
app.listen(PORT, () => {
  console.log(`MyFitAI backend (Groq) escuchando en el puerto ${PORT}`);
});
