...
async function callMistral(messages, extra = {}){
  const res = await fetch("https://api.mistral.ai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: "pixtral-12b-2409", messages, ...extra }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error?.message || data.message || "Error de Mistral");
  return data;
}
...
// Modelos gratis con visión de respaldo, por si Mistral no responde
// (límite de peticiones, mantenimiento, etc). Se prueban en orden.
const FALLBACK_SCAN_MODELS = [
  "google/gemma-4-31b-it:free",
  "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free",
  "meta-llama/llama-3.2-11b-vision-instruct:free",
];

/** Analiza la imagen con Mistral primero; si falla, prueba OpenRouter en cadena. */
async function analyzeImage(prompt, mimeType, imageBase64){
  const dataUri = `data:${mimeType || "image/jpeg"};base64,${imageBase64}`;

  // 1) Mistral (proveedor propio, estable)
  try {
    ...
    if (text) return text;
  } catch { /* seguimos con los de respaldo */ }

  // 2) OpenRouter, probando varios modelos gratis en cadena
  let lastErr;
  for (const model of FALLBACK_SCAN_MODELS) {
    try {
  ...
  throw lastErr || new Error("Ningún proveedor de IA respondió.");
}
...
markdown ni backticks:
{
  ...
  "items": [
    ...
    const text = await analyzeImage(prompt, mimeType, imageBase64);
